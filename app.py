...
TRANSLATION_SYSTEM_PROMPT = """You are a professional technical translator and QA documentation editor.
You will be given a JSON array of software bug reports. Some fields are in Arabic, some in English, some mixed within the same field.

For EVERY report, produce an English version of "title", "description", and "steps" that:
- Preserves the FULL technical meaning and severity-relevant detail of the original. Do not soften, summarize, omit, or add anything that is not in the source text.
- Translates Arabic text into precise, professional English technical writing, as a senior QA engineer would write it.
- Leaves text that is already in English essentially unchanged (only fix obvious typos/OCR noise), do not paraphrase for style.
- Keeps identifiers exactly as written and untranslated: field names (e.g. is_active, form_type, validity_days), endpoint paths, HTTP status codes, error codes, UI labels in quotes, and numbers/dates.
- If a field is empty in the source, return it as an empty string. Never invent content for an empty field.
- Keeps each field on its own single string (no added bullet points or headers beyond what the source implies).

Return ONLY a valid JSON array, same length and same order as the input, with this exact shape:
[{"id": <input id>, "title": "...", "description": "...", "steps": "..."}]
...
def translate_batch(client, rows):
    """Translate one batch of (row_id, row) pairs. Returns {row_id: {title, description, steps}}.
    Falls back to original text for any row that fails to parse."""
    ...
    try:
        ...
        # Any row the model dropped falls back to its original text.
...
def translate_dataframe(df, _client):
    """Adds 'Title (EN)', 'Description (EN)', 'Steps (EN)' columns.
    Leading underscore on _client tells st.cache_data not to hash the client object."""
    df = df.copy()
    row_ids = list(df.index)
    all_rows = [(rid, df.loc[rid]) for rid in row_ids]

    translations = {}
    progress = st.progress(0.0, text="Translating reports to English...")
    n_batches = max(1, (len(all_rows) + TRANSLATION_BATCH_SIZE - 1) // TRANSLATION_BATCH_SIZE)

    for i in range(0, len(all_rows), TRANSLATION_BATCH_SIZE):
        batch = all_rows[i:i + TRANSLATION_BATCH_SIZE]
        translations.update(translate_batch(_client, batch))
        progress.progress(min(1.0, (i // TRANSLATION_BATCH_SIZE + 1) / n_batches))
