...
VERIFICATION_BATCH_SIZE = 15  # candidate matches per verification request
...
# MATCH VERIFICATION
# --------------------------------------------------------------------------
# Embedding cosine-similarity against a small, fixed KNOWN_DEFECTS list is a
# weak signal on its own: short bug reports (especially once translated
# across languages) can clear the similarity threshold against the "least
# bad" candidate even when they don't actually describe the same underlying
# behavior -- they just share a topic (e.g. both mention Arabic fields).
# This pass asks Gemini to confirm each embedding-proposed match before it's
# trusted as a real "known_defect" hit.

VERIFY_SYSTEM_PROMPT = """You are the verification step in an automated bug-triage pipeline. An embedding similarity search has proposed a candidate match between a bug report and a known defect from a reference list. Your job is to confirm or reject each proposed match.

For each pair:
- Read the bug report and the candidate known-defect description.
- Confirm the match ONLY if the bug report describes the SAME underlying technical behavior as the candidate -- not merely a similar topic or the same general area of the product.
- Sharing a broad theme (e.g. both mention "Arabic fields", both are about "editing a form") is NOT sufficient on its own if the actual behavior described differs.
- If the bug report is too vague or general to tell whether it's the same defect, reject the match (match: false) rather than guessing.

Return ONLY a JSON array, one object per pair, in the same order given:
[{"id": <id>, "match": true or false, "reason": "<reason, under 15 words>"}]
No markdown fences, no commentary, no extra keys.
"""


def verify_matches(client, candidates):
    """candidates: list of (row_id, bug_text, candidate_text).
    Returns {row_id: (is_match: bool, reason: str)}.
    On any API/parse failure, defaults to KEEPING the embedding match (so an
    outage doesn't silently wipe every match) but notes verification failed."""
    if not candidates:
        return {}

    payload = [
        {"id": i, "bug_report": bug_text, "candidate_defect": candidate_text}
        for i, (_row_id, bug_text, candidate_text) in enumerate(candidates)
    ]
    prompt = VERIFY_SYSTEM_PROMPT + "\n\nInput:\n" + json.dumps(payload, ensure_ascii=False)

    fallback = {
        row_id: (True, "verification unavailable -- kept embedding match")
        for row_id, _bug_text, _candidate_text in candidates
    }
    ...
    try:
        ...
        verified = {}
        for i, (row_id, _bug_text, _candidate_text) in enumerate(candidates):
            item = results[i] if i < len(results) else None
            if item is None:
                verified[row_id] = fallback[row_id]
            else:
                verified[row_id] = (bool(item.get("match", True)), item.get("reason", ""))
        return verified
    except Exception:
        return fallback


def verify_matches_batched(client, candidates, batch_size=VERIFICATION_BATCH_SIZE):
    results = {}
    for i in range(0, len(candidates), batch_size):
        results.update(verify_matches(client, candidates[i:i + batch_size]))
    return results
...
def process_bug_reports(df, n_clusters, _client):
    df = prepare_text(df)
    ...
    # Verification pass: confirm every embedding-proposed "known_defect"
    # match actually describes the same behavior before trusting it. Rows
    # that fail verification are downgraded to "new_issue" -- but the
    # rejected candidate + reason are kept in their own columns so a
    # reviewer can see what the embedding search almost matched and why
    # the AI didn't accept it.
    df["Rejected Match"] = ""
    df["Rejected Match Reason"] = ""

    candidates = [
        (idx, row["bug_text"], KNOWN_DEFECTS_BY_ID[row["match_id"]]["text"])
        for idx, row in df.iterrows()
        if row.get("status") == "known_defect" and row.get("match_id") in KNOWN_DEFECTS_BY_ID
    ]
    verification = verify_matches_batched(_client, candidates)

    for idx, (is_match, reason) in verification.items():
        if not is_match:
            df.at[idx, "Rejected Match"] = df.at[idx, "match_id"]
            df.at[idx, "Rejected Match Reason"] = reason or "AI verification rejected the embedding match"
            df.at[idx, "status"] = "new_issue"
            df.at[idx, "match_id"] = None
            df.at[idx, "severity"] = None
...
if uploaded_file is not None:
    ...
    df, embeddings, similar_pairs = process_bug_reports(raw_df, n_clusters, client)
    ...
    if "severity" in df.columns:
        ...
        # Rows where embedding similarity proposed a KNOWN_DEFECTS match but
        # the Gemini verification pass rejected it (same topic, different
        # actual behavior) -- kept visible so you can audit these decisions
        # rather than have them silently disappear into "new_issue".
        rejected_df = df[df["Rejected Match"] != ""]
        if len(rejected_df) > 0:
            with st.expander(f"🔍 AI-rejected embedding matches ({len(rejected_df)}) — for validation", expanded=False):
                st.caption(
                    "These reports were the closest embedding match to a known "
                    "defect, but the AI verification step decided the actual "
                    "behavior described doesn't match, so they were kept as "
                    "unclassified new issues instead of being forced into a "
                    "severity bucket."
                )
                rejected_display = rejected_df.copy()
                rejected_display["Rejected Defect Description"] = rejected_display["Rejected Match"].apply(matched_defect_text)
                st.dataframe(
                    rejected_display[["Title", "Description", "Group", "Rejected Match", "Rejected Defect Description", "Rejected Match Reason"]],
